...
public class MapFragmentView{
                ...
                m_mapFragment.init(new OnEngineInitListener() {
                    ...
                    public void onEngineInitializationCompleted(OnEngineInitListener.Error error) {
                        ...
                        if (error == Error.NONE) {
                            ...
                            m_map.setCenter(new GeoCoordinate(-37.876808, 145.044858), Map.Animation.NONE);
    ...
    private void createRoute() {
        ...
        //Log.e(TAG,"data_click_button"+MainActivity.jsonObject.toString());
        //Log.e(TAG,"data_click_button_within_forloop"+MainActivity.jsonObject.getJSONArray("MethodParam"));

        //Log.e(TAG,"lengthsfsdfsdf"+MainActivity.jsonObject.length());

        try {
            for(int i = 2;i<MainActivity.jsonObject.getJSONArray("methodParam").length();i++){
                JSONObject jsonObject = MainActivity.jsonObject.getJSONArray("methodParam").getJSONObject(i);
                //Log.e(TAG,"data_click_button_within_forloop"+jsonObject.toString());
                RouteWaypoint routeWaypoint = new RouteWaypoint(new GeoCoordinate(jsonObject.getDouble("latitude"),jsonObject.getDouble("longitude")));
                //if(i==0 || i == MainActivity.jsonObject.getJSONArray("methodParam").length()-1){
                if( i==2 ||i == MainActivity.jsonObject.getJSONArray("methodParam").length()-1){
                ...
                //Log.e(TAG,"data_click_button_within_forloop"+routeWaypoint.getR);
                routePlan.addWaypoint(routeWaypoint);
                //routePlan.addWaypoint(new RouteWaypoint(new GeoCoordinate(jsonObject.getDouble("latitude"),jsonObject.getDouble("longitude"))));
                //routePlan.getWaypoint(i).setWaypointType(RouteWaypoint.Type.STOP_WAYPOINT);
            }
        } catch (JSONException e) {
            Log.e(TAG,"Try error2");
        }
        //routePlan.getWaypoint(0).setWaypointType(RouteWaypoint.Type.STOP_WAYPOINT);
        //routePlan.getWaypoint(routePlan.getWaypointCount()-1).setWaypointType(RouteWaypoint.Type.STOP_WAYPOINT);
                    ...
                    public void onCalculateRouteFinished(List<RouteResult> routeResults,
                        ...
                        if (routingError == RoutingError.NONE) {
                            if (routeResults.get(0).getRoute() != null) {
                                ...
                                m_map.setTilt(80);
                                //m_map.setTilt(80, Map.Animation.LINEAR);
